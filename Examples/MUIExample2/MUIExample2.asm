...
start:
    ...
    ;-----------------------------------------------------------------------------------------------------
    ; Register all ModernUI controls that are used in dialog resources
    ;-----------------------------------------------------------------------------------------------------
...
InitGUI PROC hWin:DWORD
    ...
    ; NOTES
    ;-----------------------------------------------------------------------------------------------------
    ; This example, whilst similar to MUIExample1, makes use of dialog resources instead of programmatically
    ; creating them. Each dialog panel has the appropriate ModernUI control already placed on them.
    ; 
    ; Each dialog panel is registered with the ModernUI_SmartPanel control, which initializes each dialog.
    ;
    ; At initialization of each dialog, any child controls are also created, including our ModernUI controls.
    ;
    ; We then fetch the handle to these controls with the GetDlgItem call, then we are able to set some
    ; properties of each control like we do in the MUIExample1 project.
    ;-----------------------------------------------------------------------------------------------------
    ...
    ; ModernUI_SmartPanel
    ; ------------------------------------------------------------------------
    Invoke GetDlgItem, hWin, IDC_SMARTPANEL
    ...
    ;-----------------------------------------------------------------------------------------------------        
    Invoke GetDlgItem, hWin, IDC_TEXTRSHEADER
    ;Invoke MUITextCreate, hWin, Addr szRSHeader, 17, 70, 457, 30, IDC_TEXTRSHEADER, MUITS_13PT_BOLD  or MUITS_FONT_SEGOE 
